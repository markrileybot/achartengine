...
public abstract class XYChart extends AbstractChart {
	...
	private final Rect mScreenR = new Rect();
	...
	private double[] maxX = new double[0];
	private double[] minY = new double[0];
	private double[] maxY = new double[0];
	private boolean[] isMinXSet = new boolean[0];
	private boolean[] isMaxXSet = new boolean[0];
	private boolean[] isMinYSet = new boolean[0];
	private boolean[] isMaxYSet = new boolean[0];
	...
	public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
		...
		prepareMinsAndMaxes(maxScaleNumber);
		...
		if (showGridX || showGridY) {
			...
			List<Double> xLabels = getXLabels(minX[0], maxX[0], mRenderer.getXLabels());
		...
		if (showLabels) {
			...
			drawLabels(canvas, paint, left, right, bottom, or, maxScaleNumber, minY, maxY, yPixelsPerUnit, showTickMarks, showCustomTextGridY);
			drawTitle(canvas, x, y, width, height, paint, top, right, bottom, or, maxScaleNumber);
		}
		if (or == Orientation.HORIZONTAL) {
	...
	private void prepareMinsAndMaxes(int maxScaleNumber) {
		if(minX.length != maxScaleNumber) {
			minX = new double[maxScaleNumber];
			maxX = new double[maxScaleNumber];
			minY = new double[maxScaleNumber];
			maxY = new double[maxScaleNumber];
			isMinXSet = new boolean[maxScaleNumber];
			isMaxXSet = new boolean[maxScaleNumber];
			isMinYSet = new boolean[maxScaleNumber];
			isMaxYSet = new boolean[maxScaleNumber];
			xPixelsPerUnit = new double[maxScaleNumber];
			yPixelsPerUnit = new double[maxScaleNumber];
		}
	}

	private void drawLabels(Canvas canvas, Paint paint, int left, int right, int bottom, Orientation or, int maxScaleNumber, double[] minY, double[] maxY, double[] yPixelsPerUnit, boolean showTickMarks, boolean showCustomTextGridY) {
	...
	private void drawTitle(Canvas canvas, int x, int y, int width, int height, Paint paint, int top, int right, int bottom, Orientation or, int maxScaleNumber) {
		paint.setColor(mRenderer.getLabelsColor());
	...
	private void drawAxes(Canvas canvas, Paint paint, int left, int top, int right, int bottom, Orientation or, int maxScaleNumber) {
